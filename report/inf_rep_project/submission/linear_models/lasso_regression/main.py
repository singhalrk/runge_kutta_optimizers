...
parser.add_argument('--model', default='Logistic_Regression', type=str)
...
def main() :
    ...
    def train(epoch) :
        ...
        if epoch in epoch_step:
            lr_new = opt.lr * (opt.lr_decay**(epoch_step.index(epoch) + 1))
            optimizer = get_optimizer(net,opt, lr_new)
            print('lr updated to %s'%lr_new)
        else:
            optimizer = get_optimizer(net, opt, opt.lr)
