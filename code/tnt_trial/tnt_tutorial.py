...
from mod_rk2_trial import RK2
from newProgress_trial import KeepProgress
...
def main():
    ...
    optimizer = RK2(params.values(), lr=0.01,  weight_decay=0.0005)
    # optimizer = torch.optim.SGD(params.values(), lr=0.01,  weight_decay=0.0005)
    ...
    def on_start(state):
        state['progress'] = KeepProgress(state['network'],{'lr':0.1, 'optimizer':'SGD'})
