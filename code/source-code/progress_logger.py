...
class KeepProgress():
    ...
    def __init__(self,net,args, base,grad_norm = None) :
        ...
        self.epochs = 0
        self.train_plots = 0
        self.train_best = {'loss':1e+9, 'accuracy':0}
        self.test_best = {'loss':1e+9, 'accuracy':0}
        ...
        self.__class__.experiment_number += 1
    ...
    def train_progress(self,vals) :
        ...
        if vals['train_loss'] <= self.train_best['loss']:
            self.train_best['loss'] = vals['train_loss']

        if vals['train_accuracy'] >= self.train_best['accuracy']:
           self.train_best['accuracy'] = vals['train_accuracy']

        print("train loss = {}, train accuracy = {}".format(vals['train_loss'], vals['train_accuracy']))
    ...
    def test_progress(self,vals) :
        ...
        if vals['test_loss'] <= self.test_best['loss']:
            self.test_best['loss'] = vals['test_loss']

        if vals['test_accuracy'] >= self.test_best['accuracy']:
         self.test_best['accuracy'] = vals['test_accuracy']

        print("test loss = {}, test accuracy = {}".format(vals['test_loss'], vals['test_accuracy']))

        if self.epochs%2 == 0: self.pickle_data()

    def pickle_data(self) :
        if self.train_plots % 20 ==0:
            print(".... saving data ....")
