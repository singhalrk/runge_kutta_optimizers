...
class RK2(Optimizer):
    ...
    def step(self, closure):
        ...
        for group in self.param_groups:
            for i, q in enumerate(group['params']):
                # p_real.append(q)
                p_real.append(deepcopy(q))
                if q.grad is None:
                    grad_k1.append(None)
                    continue
                ...
                if weight_decay != 0:
                    d_p_1.add_(weight_decay, q.data)

                grad_k1.append(deepcopy(d_p_1))
                q.data.add_(-group['lr']*2/3, d_p_1)
        ...
        for group in self.param_groups:
            for j, p in enumerate(group['params']):
                ...
                # print((p.grad.data.numpy() - grad_k1[j].numpy()))
                ...
                # print((p.data.numpy() - p_real[j].data.numpy()))

                p_real[j].data.add_(-group['lr']/4, d_p)
                p.data = p_real[j].data
                p.data = p.data.add_(weight_decay, torch.randn(p.data.size()))
