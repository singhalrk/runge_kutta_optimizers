...
parser.add_argument('--optimizer', default='SGD_momentum', type=str, help='optimizer')
...
def get_optimizer(net, args):
    ...
    if args.optimizer == 'SGD_nesterov':
        ...
        optimizer = RK2(net.parameters(), args.lr, weight_decay=args.weight_decay)

    elif args.optimizer == 'RK2_heun':
        optimizer = RK2_heun(net.parameters(), args.lr, weight_decay=args.weight_decay)

    elif args.optimizer == 'RK4':
        optimizer = RK4(net.parameters(), args.lr, weight_decay=args.weight_decay)
...
def main():
    ...
    # create logger
    basefile = '{}_{}'.format(random.randint(1,100000), random.randint(1,100000))
    progress = KeepProgress(model, args, basefile)
    print(basefile)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        prec1 = validate(val_loader, model, criterion, progress)
...
def train(train_loader, model, criterion, optimizer, epoch, progress):
    ...
    for i, (input, target) in enumerate(train_loader):
        ...
        def closure():
            model.zero_grad()
            loss = criterion(model(input_var), target_var)
            loss.backward()
            return loss
        ...
        if i % args.print_freq == 0:
            ...
            vals = {'train_loss': losses.val, 'train_accuracy': top1.val}
            progress.train_progress(vals)

def validate(val_loader, model, criterion, progress):
    batch_time = AverageMeter()
    losses = AverageMeter()
    top1 = AverageMeter()
    top5 = AverageMeter()
    ...
    for i, (input, target) in enumerate(val_loader):
        ...
        if i % args.print_freq == 0:
            ...
            # vals = {'test_loss': losses.val, 'test_accuracy':top1.avg}
            # progress.test_progress(vals)

    print(' * Prec@1 {top1.avg:.3f} Prec@5 {top5.avg:.3f}'
          .format(top1=top1, top5=top5))
    vals = {'test_loss': losses.avg, 'test_accuracy': top1.avg}
    progress.test_progress(vals)
    return top1.avg
