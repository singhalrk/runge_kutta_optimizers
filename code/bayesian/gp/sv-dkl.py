"""
An example to use Gaussian Process (GP) module to classify MNIST. Follow the
idea from reference [1], we will combine a convolutional neural
network with a RBF kernel to create a "deep" kernel.
Then we train a SparseVariationalGP model using SVI. Note
...
from bayesian.rk2_heun import RK2_heun
from bayesian.rk2_ralston import RK2
# from bayesian.logger import KeepProgress
...
def get_optimizer(args, params):
    if args.optimizer == 'Adam':
        return optim.Adam({"lr": args.lr})
    elif args.optimizer == 'RK2':
        return RK2(params=params, lr=args.lr, weight_decay=1e-5)
    elif args.optimizer == 'RK2_heun':
        return RK2_heun({"lr": args.lr, "weight_decay": 1e-5})
    elif args.optimizer == 'SGD':
        return optim.SGD({"lr": args.lr, "weight_decay": 1e-5})
    elif args.optimizer == 'ASGD':
        return optim.ASGD({"lr": args.lr})
    elif args.optimizer == 'Adadelta':
        return optim.Adadelta({"lr": args.lr})
...
def main(args):
    ...
    svi = infer.SVI(gpmodel.model, gpmodel.guide,
                    optimizer, infer.Trace_ELBO())
